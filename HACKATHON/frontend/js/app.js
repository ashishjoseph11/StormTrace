...
async function updateGeeLayers() {
  try {
    const res = await fetch('/api/satellite/layers');
...
async function updateSurgeLayer() {
  ...
  try {
    const res = await fetch(`/api/hazards/surge?scenario=${state.activeSurgeScenario}`);
...
async function openDiffModal() {
  ...
  try {
    const res = await fetch('/api/compare?from_update=update-01&to_update=update-03');
    const data = await res.json();
